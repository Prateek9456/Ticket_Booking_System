...
function createPasswordResetOtp(db, email) {
...
function verifyPasswordResetOtp(db, email, otp) {
  const normalized = normalizeEmail(email);
  ...
  db.prepare("UPDATE email_otps SET used_at = datetime('now') WHERE id = ?").run(row.id);
  return true;
}
